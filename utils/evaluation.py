...
def fair_metric(pred, cf_pred, labels, sens):
    ...
    return parity.item(), equality.item(), counterfactual_fairness
...
def calc_metrics(output, counter_output, data, count):
    ...
    accs, auc_rocs, F1s, paritys, equalitys, counterfactual_fairness = {}, {}, {}, {}, {}, {}
    ...
    paritys['val'], equalitys['val'], counterfactual_fairness['val'] = fair_metric(pred_val.cpu().numpy(),cf_pred_val.cpu().numpy(), data.y[data.val_mask[count]].cpu(
    ).numpy(), data.sens[data.val_mask[count]].cpu().numpy())
    paritys['test'], equalitys['test'], counterfactual_fairness['test'] = fair_metric(pred_test.cpu().numpy(), cf_pred_test.cpu().numpy(), data.y[data.test_mask[count]].cpu(
    ).numpy(), data.sens[data.test_mask[count]].cpu().numpy())

    return accs, auc_rocs, F1s, paritys, equalitys, counterfactual_fairness
...
def evaluate(x, classifier, hp, encoder, data, args, count):
    ...
    with torch.no_grad():
        ...
        output = classifier(h)
        counter_h = encoder(args.counter_features)
        counter_output = classifier(counter_h)        

    accs, auc_rocs, F1s, paritys, equalitys, counterfactual_fairness = calc_metrics(output, counter_output, data, count)

    labels = data.y[data.test_mask[count]].cpu().numpy()
    pred = (output[data.test_mask[count]].squeeze() > 0).type_as(data.y).cpu().numpy()
    sens = data.sens[data.test_mask[count]].cpu().numpy()

    return accs, auc_rocs, F1s, paritys, equalitys, counterfactual_fairness, labels, pred, sens