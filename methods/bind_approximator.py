...
def wasserstein_cost_loss(device, output, sens):

    group_1_diff = output[sens==1]
    group_0_diff = output[sens==0]

    return wasserstein(group_0_diff, group_1_diff, device)

def grad_z_graph_faircost(device, adj, features, idx, labels, sens, model):
    model.eval()
    adj, features, idx, labels, sens, model = \
        adj.to(device), features.to(device), idx.to(device), labels.to(device), sens.to(device), model.to(device)

    output = model(features, adj)

    if output.shape[1] == 1:
        loss1 = wasserstein_cost_loss(device, output[idx], sens[idx])
        loss2 = wasserstein_cost_loss(device, output[idx][labels[idx] == 1], sens[idx][labels[idx] == 1])
...
def s_test_graph_cost(device, adj, features, idx_train, idx_test, labels, sens, model, damp=0.03, scale=60, recursion_depth=5000):
    v = grad_z_graph_faircost(device, adj, features, idx_test, labels, sens, model)
    h_estimate_cost = v.copy()

    adj, features, idx_train, idx_test, labels, sens, model = \
        adj.to(device), features.to(device), idx_train.to(device), idx_test.to(device), labels.to(device), sens.to(device), model.to(device)
...
def grad_z_graph(device, adj, features, idx, labels, model):

    model.eval()
    adj, features, labels, model = adj.to(device), features.to(device), labels.to(device), model.to(device)
...
def cal_influence_graph(idx_train, h_estimate_mean, gradients_list):
...
def cal_influence_graph_nodal(idx_all, idx_train, h_estimate_mean, gradients_list):
